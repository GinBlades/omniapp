  ...
  class ActivitiesController < ::AdminController
    ...
    def create
      @points_activity = if params[:option_id]
        ...
        Activity.new(points_activity_params)
      end
    ...
    private
      def set_points_goal
        @points_goal = params[:goal_id] ? Goal.find(params[:goal_id]) : nil
      end

      def set_points_option
        @points_option = params[:option_id] ? Option.find(params[:option_id]) : nil
      end

      def set_points_activity
        @points_activity = Activity.find(params[:id])
      end
